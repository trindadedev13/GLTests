...
/* glm__noiseDetail_gradNorm_vec4(vec4 g00__, vec4 g01__, vec4 g10__, vec4
 * g11__) */
#define glm__noiseDetail_gradNorm_vec4(g00__, g01__, g10__, g11__)         \
  {                                                                        \
    ...
    glm__noiseDetail_taylorInvSqrt(norm,                                   \
                                   norm); /* norm = taylorInvSqrt(norm) */ \
                                                                           \
...
float glm_perlin_vec4(vec4 point) {
  ...
  vec4 g0000 = {gx00[0], gy00[0], gz00[0],
                gw00[0]}; /* g0000 = vec4(gx00.x, gy00.x, gz00.x, gw00.x); */
  vec4 g0100 = {gx00[2], gy00[2], gz00[2],
                gw00[2]}; /* g0100 = vec4(gx00.z, gy00.z, gz00.z, gw00.z); */
  vec4 g1000 = {gx00[1], gy00[1], gz00[1],
                gw00[1]}; /* g1000 = vec4(gx00.y, gy00.y, gz00.y, gw00.y); */
  vec4 g1100 = {gx00[3], gy00[3], gz00[3],
                gw00[3]}; /* g1100 = vec4(gx00.w, gy00.w, gz00.w, gw00.w); */

  vec4 g0001 = {gx01[0], gy01[0], gz01[0],
                gw01[0]}; /* g0001 = vec4(gx01.x, gy01.x, gz01.x, gw01.x); */
  vec4 g0101 = {gx01[2], gy01[2], gz01[2],
                gw01[2]}; /* g0101 = vec4(gx01.z, gy01.z, gz01.z, gw01.z); */
  vec4 g1001 = {gx01[1], gy01[1], gz01[1],
                gw01[1]}; /* g1001 = vec4(gx01.y, gy01.y, gz01.y, gw01.y); */
  vec4 g1101 = {gx01[3], gy01[3], gz01[3],
                gw01[3]}; /* g1101 = vec4(gx01.w, gy01.w, gz01.w, gw01.w); */

  vec4 g0010 = {gx10[0], gy10[0], gz10[0],
                gw10[0]}; /* g0010 = vec4(gx10.x, gy10.x, gz10.x, gw10.x); */
  vec4 g0110 = {gx10[2], gy10[2], gz10[2],
                gw10[2]}; /* g0110 = vec4(gx10.z, gy10.z, gz10.z, gw10.z); */
  vec4 g1010 = {gx10[1], gy10[1], gz10[1],
                gw10[1]}; /* g1010 = vec4(gx10.y, gy10.y, gz10.y, gw10.y); */
  vec4 g1110 = {gx10[3], gy10[3], gz10[3],
                gw10[3]}; /* g1110 = vec4(gx10.w, gy10.w, gz10.w, gw10.w); */

  vec4 g0011 = {gx11[0], gy11[0], gz11[0],
                gw11[0]}; /* g0011 = vec4(gx11.x, gy11.x, gz11.x, gw11.x); */
  vec4 g0111 = {gx11[2], gy11[2], gz11[2],
                gw11[2]}; /* g0111 = vec4(gx11.z, gy11.z, gz11.z, gw11.z); */
  vec4 g1011 = {gx11[1], gy11[1], gz11[1],
                gw11[1]}; /* g1011 = vec4(gx11.y, gy11.y, gz11.y, gw11.y); */
  vec4 g1111 = {gx11[3], gy11[3], gz11[3],
                gw11[3]}; /* g1111 = vec4(gx11.w, gy11.w, gz11.w, gw11.w); */
  ...
  /* n_0w = lerp(vec4(n0000, n1000, n0100, n1100), vec4(n0001, n1001, n0101,
   * n1101), fade_xyzw.w) */
  ...
  /* n_1w = lerp(vec4(n0010, n1010, n0110, n1110), vec4(n0011, n1011, n0111,
   * n1111), fade_xyzw.w) */
...
float glm_perlin_vec3(vec3 point) {
  ...
  /* n_z = lerp(vec4(n000, n100, n010, n110), vec4(n001, n101, n011, n111),
   * fade_xyz.z); */
  vec4 n_z;
...
float glm_perlin_vec2(vec2 point) {
  /* Integer part of p for indexing */
  /* Pi = floor(vec4(point.x, point.y, point.x, point.y)) + vec4(0.0,
   * 0.0, 1.0, 1.0); */
  vec4 Pi = {point[0], point[1], point[0],
             point[1]};   /* Pi = vec4(point.x, point.y, point.x, point.y) */
  glm_vec4_floor(Pi, Pi); /* Pi = floor(Pi) */
  Pi[2] += 1.0f;          /* Pi.z += 1.0 */
  Pi[3] += 1.0f;          /* Pi.w += 1.0 */

  /* Fractional part of p for interpolation */
  /* vec<4, T, Q> Pf = glm::fract(vec<4, T, Q>(Position.x, Position.y,
   * Position.x, Position.y)) - vec<4, T, Q>(0.0, 0.0, 1.0, 1.0); */
  vec4 Pf = {point[0], point[1], point[0],
             point[1]};   /* Pf = vec4(point.x, point.y, point.x, point.y) */
  ...
  vec4 ix = {Pi[0], Pi[2], Pi[0],
             Pi[2]}; /* ix = vec4(Pi.x, Pi.z, Pi.x, Pi.z) */
  vec4 iy = {Pi[1], Pi[1], Pi[3],
             Pi[3]}; /* iy = vec4(Pi.y, Pi.y, Pi.w, Pi.w) */
  vec4 fx = {Pf[0], Pf[2], Pf[0],
             Pf[2]}; /* fx = vec4(Pf.x, Pf.z, Pf.x, Pf.z) */
  vec4 fy = {Pf[1], Pf[1], Pf[3],
             Pf[3]}; /* fy = vec4(Pf.y, Pf.y, Pf.w, Pf.w) */
  ...
  glm_vec2_lerp(n_x1, n_x2, fade_xy[0],
                n_x); /* n_x = lerp(n_x1, n_x2, fade_xy.x) */
