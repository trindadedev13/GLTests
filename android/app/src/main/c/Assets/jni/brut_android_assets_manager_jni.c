#include "brut_android_assets_manager_jni.h"
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
  ...
  asmgr->jvm = vm;
  asmgr->o_brut_assets_manager = NULL;
  asmgr->c_brut_assets_manager = NULL;
...
Java_dev_trindadedev_brut_BrutActivity_initAssets(JNIEnv* env,
  ...
  if (asmgr->o_brut_assets_manager != NULL)
    (*env)->DeleteGlobalRef(env, asmgr->o_brut_assets_manager);

  if (asmgr->c_brut_assets_manager != NULL)
    (*env)->DeleteGlobalRef(env, asmgr->c_brut_assets_manager);

  asmgr->o_brut_assets_manager = (*env)->NewGlobalRef(env, mgr);

  jclass localClass = (*env)->GetObjectClass(env, asmgr->o_brut_assets_manager);
  asmgr->c_brut_assets_manager = (jclass)(*env)->NewGlobalRef(env, localClass);
  (*env)->DeleteLocalRef(env, localClass);
}

static JNIEnv* BrutAssetsManagerJNI_GetEnv() {
...
char* BrutAssetsManagerJNI_ReadTextFile(const char* filepath) {
...
// Calls BrutAssetsManager.java#fileExists with JNI
bool BrutAssetsManagerJNI_FileExists(const char* filepath) {
  JNIEnv* env = BrutAssetsManagerJNI_GetEnv();
  if (env == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to get JNIEnv*");
    return false;
