...
public class SDLControllerManager {
  ...
  static native void nativeAddJoystick(int device_id, String name, String desc, int vendor_id,
          int product_id, int button_mask, int naxes, int axis_mask, int nhats, boolean can_rumble);
   ...
   * This method is called by SDL using JNI.
   */
  static void hapticRumble(int device_id, float low_frequency_intensity,
          float high_frequency_intensity, int length) {
...
class SDLJoystickHandler_API16 extends SDLJoystickHandler {
  ...
  static class RangeComparator implements Comparator<InputDevice.MotionRange> {
    ...
    public int compare(InputDevice.MotionRange arg0, InputDevice.MotionRange arg1) {
      // Some controllers, like the Moga Pro 2, return AXIS_GAS (22) for right trigger and
      // AXIS_BRAKE (23) for left trigger - swap them so they're sorted in the right order for SDL
      ...
      if (SDLControllerManager.isDeviceSDLJoystick(device_id)) {
        ...
        if (joystick == null) {
          ...
          SDLControllerManager.nativeAddJoystick(joystick.device_id, joystick.name, joystick.desc,
                  ...
                  joystick.hats.size() / 2, can_rumble);
        }
      }
    }
    ...
    if (action == MotionEvent.ACTION_MOVE) {
      ...
      if (joystick != null) {
        ...
        for (int i = 0; i < joystick.hats.size() / 2; i++) {
          int hatX = Math.round(
                  event.getAxisValue(joystick.hats.get(2 * i).getAxis(), actionPointerIndex));
          int hatY = Math.round(
                  event.getAxisValue(joystick.hats.get(2 * i + 1).getAxis(), actionPointerIndex));
...
class SDLHapticHandler_API31 extends SDLHapticHandler {
  ...
  @Override
  void rumble(int device_id, float low_frequency_intensity, float high_frequency_intensity,
          int length) {
