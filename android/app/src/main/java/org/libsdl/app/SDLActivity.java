...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
  ...
  */
  ...
  /**
   * The application entry point, called on a dedicated thread (SDLThread). The default
   * implementation uses the getMainSharedObject() and getMainFunction() methods to invoke native
   * code from the specified shared library. It can be overridden by derived classes.
   */
  protected void main() {
  ...
  public static void initialize() {
    // The static nature of the singleton and Android quirkyness force us to initialize everything
    // here
    // Otherwise, when exiting the app and returning to it, these variables *keep* their pre exit
    // values
    mSingleton = null;
    mSurface = null;
    mTextEdit = null;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (mBrokenLibraries) {
      ...
      dlgAlert.setMessage(
          "An error occurred while trying to start the application. Please try again and/or"
              + " reinstall."
  ...
  protected void onDestroy() {
    ...
    if (SDLActivity.mSDLThread != null) {
      ...
      try {
        ...
        // and so the previous QUIT event will be discarded by SDL_Init() and app is running, not
        // exiting.
  ...
  public boolean dispatchKeyEvent(KeyEvent event) {
    ...
    if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
        ...
        || /* API 11 */ keyCode == KeyEvent.KEYCODE_ZOOM_OUT /* API 11 */) {
      return false;
    }
  ...
  /**
   * This method is called by SDL if SDL did not handle a message itself. This happens if a received
   * message contains an unsupported command. Method can be overwritten to handle Messages in a
   * different class.
   *
  ...
  /**
   * A Handler class for Messages from native SDL applications. It uses current Activities as target
   * (e.g. for the title). static to prevent implicit references to enclosing object.
   */
  ...
  public static native void onNativeTouch(
      int touchDevId, int pointerFingerId, int action, float x, float y, float p);

  public static native void onNativePen(
      int penId, int button, int action, float x, float y, float p);
  ...
  /**
   * This method is called by SDL using JNI. This is a static method for JNI convenience, it calls a
   * non-static method so that is can be overridden
   */
  ...
  public static boolean isScreenKeyboardShown() {
    ...
    InputMethodManager imm =
        (InputMethodManager) SDL.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
  ...
  public static boolean handleKeyEvent(View v, int keyCode, KeyEvent event, InputConnection ic) {
    ...
    // So, we try to process them as JOYSTICK/DPAD/GAMEPAD events first, if that fails we try them
    // as KEYBOARD
    //
  ...
  public static int createCustomCursor(
      int[] colors, int width, int height, int hotSpotX, int hotSpotY) {
  ...
  public static int openFileDescriptor(String uri, String mode) throws Exception {
    ...
    try {
      ParcelFileDescriptor pfd =
          mSingleton.getContentResolver().openFileDescriptor(Uri.parse(uri), mode);
  ...
  public static boolean showFileDialog(
      String[] filters, boolean allowMultiple, boolean forWrite, int requestCode) {
    if (mSingleton == null) {
      return false;
    }
    ...
    Intent intent =
        new Intent(forWrite ? Intent.ACTION_CREATE_DOCUMENT : Intent.ACTION_OPEN_DOCUMENT);
