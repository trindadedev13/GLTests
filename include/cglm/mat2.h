 ...
 */
CGLM_INLINE
void glm_mat2_make(const float* __restrict src, mat2 dest) {
  dest[0][0] = src[0];
  dest[0][1] = src[1];
  dest[1][0] = src[2];
  dest[1][1] = src[3];
 ...
 *
 * @param[in, out] mats Array of mat2’s (must be aligned (16/32) if alignment is
 * not disabled)
 * @param[in]      count Array size of mats or number of matrices
 */
CGLM_INLINE
void glm_mat2_identity_array(mat2* __restrict mats, size_t count) {
  CGLM_ALIGN_MAT mat2 t = GLM_MAT2_IDENTITY_INIT;
  size_t i;
 ...
 *
 *        m1, m2 and dest matrices can be same matrix, it is possible to write
 * this:
 *
 * @code
 ...
 */
CGLM_INLINE
void glm_mat2_transpose_to(mat2 mat, mat2 dest) {
#if defined(__wasm__) && defined(__wasm_simd128__)
