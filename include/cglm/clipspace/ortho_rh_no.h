 ...
 */
CGLM_INLINE
void glm_ortho_rh_no(float left, float right, float bottom, float top,
...
void glm_ortho_aabb_rh_no(vec3 box[2], mat4 dest) {
  glm_ortho_rh_no(box[0][0], box[1][0], box[0][1], box[1][1], -box[1][2],
                  -box[0][2], dest);
}
...
void glm_ortho_aabb_p_rh_no(vec3 box[2], float padding, mat4 dest) {
  glm_ortho_rh_no(box[0][0] - padding, box[1][0] + padding, box[0][1] - padding,
...
void glm_ortho_aabb_pz_rh_no(vec3 box[2], float padding, mat4 dest) {
  glm_ortho_rh_no(box[0][0], box[1][0], box[0][1], box[1][1],
...
void glm_ortho_default_s_rh_no(float aspect, float size, mat4 dest) {
  if (aspect >= 1.0f) {
    glm_ortho_rh_no(-size * aspect, size * aspect, -size, size, -size - 100.0f,
                    size + 100.0f, dest);
    return;
  }

  glm_ortho_rh_no(-size, size, -size / aspect, size / aspect, -size - 100.0f,
                  size + 100.0f, dest);
}
